...
// mag
static float roll_origin = 0.0f;  // 추가
static float pitch_origin = 0.0f; // 추가
...
void gyroServiceSetOrigin(float abs_r, float abs_p, float abs_y) {
    ...
    cliPrintf("[IMU] New Origin Set -> R:%.1f, P:%.1f, Y:%.1f\r\n", abs_r, abs_p, abs_y);
}
// /**
...
static void IMU_ComputeAngles_Service(Gyro_Data_t *imu, Mag_Data_t *mag, float dt) {
    ...
    // [중요] abs_yaw 변수를 여기서 먼저 선언하고 계산합니다.
    float abs_yaw = 0.0f;
    if (mag != NULL) {
        abs_yaw = atan2f((float)mag->mag_y, (float)mag->mag_x) * RAD_TO_DEG;
        if (abs_yaw < 0) abs_yaw += 360.0f;
    }

    // 2. 영점이 안 잡혔을 때 초기화 (Snap 로직)
    if (!is_origin_set) {
        imu->roll = acc_roll;
        imu->pitch = acc_pitch;
        gyroServiceSetOrigin(acc_roll, acc_pitch, abs_yaw); 
    } else {
        // 상보 필터 가동
        imu->roll  = IMU_ALPHA * (imu->roll + gyro_rate_x * dt) + (1.0f - IMU_ALPHA) * acc_roll;
        imu->pitch = IMU_ALPHA * (imu->pitch + gyro_rate_y * dt) + (1.0f - IMU_ALPHA) * acc_pitch;

        float relative_yaw = abs_yaw - yaw_origin;
        if (relative_yaw > 180.0f)  relative_yaw -= 360.0f;
        if (relative_yaw < -180.0f) relative_yaw += 360.0f;
    ...
    // 3. 최종 출력: "절대 각도 - 영점" 수행
    latest_roll  = imu->roll - roll_origin;
    latest_pitch = imu->pitch - pitch_origin;
    latest_yaw   = filtered_rel_yaw;
...
// 🎯 [핵심 추가] 내부에서 사용하는 최신값 변수들도 0으로 초기화해야 합니다.
    latest_roll = 0.0f;
    latest_pitch = 0.0f;
    latest_yaw = 0.0f;
